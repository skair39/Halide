...
struct HalideFuncs {
    ...
    int (*render)(const void*, buffer_t*, buffer_t*);
};
...
{
    const char* mode = using_metal ? "Metal" : "CPU";
    const char* other = !using_metal ? "Metal" : "CPU";
    char log_text[2048];
...
- (void)renderOneFrame: (const HalideFuncs &) halide_funcs using_metal: (bool) using_metal
{
    ...
    halide_funcs.render((__bridge void *)self, buf2, pixel_buf);
...
- (void)initiateRender
{
    ...
    int image_width = self.window.frame.size.width * f;
    int image_height = self.window.frame.size.height * f;
